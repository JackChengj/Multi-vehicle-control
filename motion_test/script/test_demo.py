...
def follow_stright_line(start_point,end_point):
    v_prefer = max_speed/2
    distance = math.sqrt((end_point[0]-start_point[0])**2+(end_point[1]-start_point[1])**2)
    T = distance/v_prefer
    number_of_path = T/d_T
...
def PID(current_target_x,current_target_y,current_target_theta):
    ...
    car_frame_point = R @ world_frame_point
    car_frame_error = R @ error_i
...
def main():
    ...
    try:
        ...
        while not rospy.is_shutdown():
            x_current,y_current,theta_current = get_current_position()
            error_i[0] = error_i[0] + current_target_x - x_current
            error_i[1] = error_i[1] + current_target_y - y_current
            error_i[2] = error_i[2] + current_target_theta - theta_current
            current_target_x = x_points[cnt]
            current_target_y = y_points[cnt]
            current_target_theta = theta_points[cnt]
            x_points,y_points,theta_points = follow_stright_line(start_point,end_point)
